    ...
    <div class="form-container">
        <form method="POST" class="admin-form">
            <div class="form-group">
                <label>Titre*</label>
                <input type="text" name="titre" value="<?= $isEdit ? htmlspecialchars($event->getTitre()) : '' ?>" required>
            </div>

            <div class="form-group">
                <label>Type*</label>
                <input type="text" name="type" value="<?= $isEdit ? htmlspecialchars($event->getType()) : '' ?>" required>
            </div>

            <div class="form-group">
                <label>Description*</label>
                <textarea name="description" rows="6" required><?= $isEdit ? htmlspecialchars($event->getDescription()) : '' ?></textarea>
            </div>

            <div class="form-group">
                <label>Image principale (chemin)*</label>
                <input type="text" name="image_main" value="<?= $isEdit ? htmlspecialchars($event->getImageMain()) : '' ?>" required>
                <small>Exemple: <?= BASE_URL ?>/uploads/events/image.png</small>
            </div>

            <div class="form-group">
                <label>Image secondaire (chemin)*</label>
                <input type="text" name="image_second" value="<?= $isEdit ? htmlspecialchars($event->getImageSecond()) : '' ?>" required>
