...
class EmailHelper {
    ...
    public static function sendEmail($to, $toName, $subject, $body, $isHTML = true) {
        error_log("📧 Tentative envoi email - To: $to, Subject: $subject");
        ...
        try {
            ...
            error_log("📧 Configuration SMTP - Host: " . SMTP_HOST . ", Port: " . SMTP_PORT . ", User: " . SMTP_USERNAME);
            
            // Debug
            if (EMAIL_DEBUG) {
                $mail->SMTPDebug = 2;
                error_log("📧 Mode debug SMTP activé");
            }
            
            // Expéditeur
            $mail->setFrom(EMAIL_FROM, EMAIL_FROM_NAME);
            error_log("📧 Expéditeur configuré - From: " . EMAIL_FROM . ", Name: " . EMAIL_FROM_NAME);
            
            // Destinataire
            $mail->addAddress($to, $toName);
            error_log("📧 Destinataire ajouté - To: $to, Name: $toName");
            
            // Contenu
            $mail->isHTML($isHTML);
            $mail->Subject = $subject;
            $mail->Body = $body;
            error_log("📧 Contenu configuré - HTML: " . ($isHTML ? 'true' : 'false'));
    ...
    public static function sendThankYouEmail($donData, $receiptPath = null) {
        ...
        // Utiliser la méthode sendEmailWithAttachment si il y a un reçu, sinon sendEmail
