...
class IAConseilGenerator
{
    ...
    {
        // Vous pouvez utiliser OpenAI, Hugging Face, ou une autre API
        // Pour OpenAI : https://platform.openai.com/api-keys
        $this->apiKey = getenv('OPENAI_API_KEY') ?: 'votre_cle_api_ici';
     ...
     */
    public function genererConseils(ReponseFormulaire $reponse): array
    {
     ...
     */
    private function preparerContexte(ReponseFormulaire $reponse): array
    {
        return [
     ...
     */
    private function genererConseilEau(array $contexte): string
    {
...
CONSEIL PERSONNALISÉ :";
     ...
     */
    private function genererConseilEnergie(array $contexte): string
    {
...
- Adapte au type de chauffage (électrique, gaz, fioul, bois)
...
CONSEIL PERSONNALISÉ :";
     ...
     */
    private function genererConseilTransport(array $contexte): string
    {
        $emissionAnnuelle = 0;
...
CONSEIL PERSONNALISÉ :";
     ...
     */
    private function appelIA(string $prompt, string $type, array $contexte): string
    {
        // Si pas de clé API, utiliser des conseils par défaut intelligents
        if ($this->apiKey === 'votre_cle_api_ici' || empty($this->apiKey)) {
     ...
     */
    private function conseilParDefaut(string $type, array $contexte): string
    {
        switch ($type) {
            case 'eau':
            ...
            case 'energie':
                ...
                if ($contexte['tempHiver'] > 21) {
                    ...
                    return $variations[array_rand($variations)];
                } elseif ($contexte['chauffageType'] === 'fioul') {
                    $variations = [
                        "Chauffage fioul : isolation prioritaire pour réduire la consommation !",
                        "Fioul + {$contexte['tempHiver']}°C : chaque degré compte, isolez d'abord !",
                        "Optimisez votre fioul : entretien chaudière + isolation = économies !",
                        "Fioul : programmation + isolation = réduction significative !"
                    ];
                    return $variations[array_rand($variations)];
                } else {
                    $variations = [
     ...
     */
    private function appelHuggingFace(string $prompt): string
    {
